<form method="POST" action="{{ route('team-members.store') }}">
    ...
    <div class="row">
        ...
        <div class="col-md-6 mb-3">
            ...
            <select class="form-select" id="year_joined" name="year_joined" required>
                ...
                @for ($year = 2018; $year <= date('Y'); $year++)
                    <option value="{{ $year }}" {{ old('year_joined') == $year ? 'selected' : '' }}>{{ $year }}</option>
                @endfor
            </select>
        </div>
        <!-- Image URL -->
        <div class="col-md-6 mb-3">
            ...
            <input type="url" class="form-control" id="image" name="image" placeholder="Enter image URL" value="{{ old('image') }}" required>
        </div>
    </div>
    <div class="row">
        <!-- Patch -->
        <div class="col-md-6 mb-3">
            ...
            <select class="form-select" id="patch" name="patch" required>
                ...
                @for ($patch = 60; $patch <= 75; $patch++)
                    <option value="{{ $patch }}" {{ old('patch') == $patch ? 'selected' : '' }}>{{ $patch }}</option>
                @endfor
            </select>
        </div>
        <!-- Role -->
        <div class="col-md-6 mb-3">
            ...
            <select class="form-select" id="role" name="role" required>
                ...
                <option value="OC" {{ old('role') === 'OC' ? 'selected' : '' }}>OC</option>
                <option value="BR" {{ old('role') === 'BR' ? 'selected' : '' }}>BR</option>
